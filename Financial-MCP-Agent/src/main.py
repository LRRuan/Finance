...
工作流程：
start_node → entity_completer ->[fundamental_analyst, technical_analyst, value_analyst, news_analyst] → summarizer → END
"""
...
from src.agents.entity_completion_agent import entity_completion_agent
...
async def main():
    ...
    try:
        ...
        # 添加五个核心智能体节点
        workflow.add_node("entity_completer", entity_completion_agent)
        ...
        workflow.add_edge("start_node", "entity_completer")

        # 添加并行执行边 - 四个分析智能体并行执行
        workflow.add_edge("entity_completer", "fundamental_analyst")
        workflow.add_edge("entity_completer", "technical_analyst")
        workflow.add_edge("entity_completer", "value_analyst")
        workflow.add_edge("entity_completer", "news_analyst")
