...
重要指令：
1.  请优先获取最新季度的财务数据。
2.  如果某个工具在查询最新季度（例如2025年第四季度）时返回“No data found”或类似错误，这很可能是因为该季度的财报尚未公布。在这种情况下，你必须**自动尝试查询上一个季度**的数据（例如，从2025年第四季度回退到2025年第三季度），直到成功获取到数据为止。
3.  不要因为一次数据获取失败就放弃整个分析。
"""
...
#获取状态 验证状态 创建llm 获取工具 创建agent 调用agent 更新状态
