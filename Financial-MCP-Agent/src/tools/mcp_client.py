...
async def get_mcp_tools():
    ...
    使用锁机制确保在多Agent并发场景下只初始化一次。

    返回:
        list: 从MCP服务器加载的LangChain兼容工具列表。
              如果初始化或工具加载失败，则返回空列表。
    """
    global _mcp_client_instance, _mcp_tools, _initialization_lock

    # 快速检查：如果已经缓存了工具，直接返回
    if _mcp_tools is not None:
        logger.debug(f"{SUCCESS_ICON} Returning cached MCP tools (already initialized).")
        return _mcp_tools

    # 使用锁防止并发初始化
    async with _initialization_lock:
        # 双重检查：在获得锁后再次检查，防止其他协程已经完成初始化
        if _mcp_tools is not None:
            logger.debug(f"{SUCCESS_ICON} Returning cached MCP tools (initialized by another coroutine).")
            return _mcp_tools
        ...
        try:
            if _mcp_client_instance is None:
                logger.info(f"{WAIT_ICON} Creating new MultiServerMCPClient instance...")
